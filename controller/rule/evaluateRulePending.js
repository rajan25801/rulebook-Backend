...
async function evaluateRulePending(req, res){
  ...
  try {
    ...
    const rulesQuery = `
        SELECT r.id, r.name, r.json_rule, r.version, r.priority,r.status, rg.execution_type
        FROM rules r
    ...
    if (ruleRes.rowCount === 0) {
      return res.status(404).json({ error: 'No pending rules found for the rule group' });
    }
...
module.exports = { evaluateRulePending };
