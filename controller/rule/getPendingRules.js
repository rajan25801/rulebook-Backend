...
async function getPendingRules(req, res) {
  try {
    const rule_group_id = req.params.rule_group_id;
    const { page = 1, limit = 20 } = req.query;
    let results;
    if (rule_group_id === 'all') {
      results = await ruleService.getAllPendingRules();
    } else {
      results = await ruleService.getPendingRules(rule_group_id);
    }
    ...
    console.error('Error getting pending rules:', error);
    return res.status(500).json({ error: 'Failed to fetch pending rules' });
  }
}

module.exports = { getPendingRules };
