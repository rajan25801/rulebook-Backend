...
async function getAllApprovedRules(req, res) {
  try {
    const { page = 1, limit = 20 } = req.query;
    const results = await ruleService.getAllApprovedRules();
    ...
    console.error('Error getting all approved rules:', error);
    return res.status(500).json({ error: 'Failed to fetch approved rules' });
  }
}

module.exports = { getAllApprovedRules };
